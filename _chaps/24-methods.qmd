...
::: content-hidden
TODO: investigate cause of incorrectly hyphenated words in PDF version
convenience and treatments were both hyphenated mid-line
:::
...
::: content-hidden
TODO: shrink all images in 2 column layout to give space
:::
...
::: content-hidden
TODO: use present tense (e.g. are identified) throughout per GH notes
:::
...
TODO: correct or eliminate the above fragment
:::
...
#### Calculated Measures {#sec-calc-meas}
...
### Primary Outcomes {#sec-primary-outs}
...
::: content-hidden
TODO: use third person (not we → the research) throughout, per GH
:::
...
This study is organized into two sessions, encompassing three distinct experiments, each aimed at evaluating one of the three measures of training effectiveness described above. This section details the methods, variables, and rationale behind the design of each experiment. For a step-by-step description of the conduct of each session, see @sec-conduct1 and @sec-conduct2.
...
Systems were configured with the Visual Studio 2019 integrated development environment (VS) as described in the MRTK setup instructions[^24-methods-17]. Unity Version Control (VCS, formerly Plastic) was used to manage all assets and source code, allowing the developers to track changes and collaborate effectively.
...
For HMDAR tracking, we relied on VE's *Area Targets*[^24-methods-20] feature, an implementation of the area-based tracking methods described in @sec-tracking. Area-based tracking compares a pre-generated 3D model of the workspace with live 3D data of the user's surroundings. This "spatial map" is a polygonal mesh generated in real-time by the HL2, using data from the depth sensor, visible light cameras, and inertial measurement unit. A sample is shown in @fig-spatmap. From this comparison, the system can determine the current position and angle of the user's head. 
...
trials and IRB versions...

2/10 - 1
(+1 -> 1)
v1.1a1 approved (proc adj)
2/17 - 1
(+1 -> 2)
v2.1 approved (I2)
2/23 - 3
3/14 - 1
(+4 -> 6)
start of steady progress
3/15 - 2
(+2 -> 8)
end of "pilot" - first 8
3/16 - 3
3/17 - 3
3/21 - 2
3/22 - 2
...
4/7 - 1
(+34 -> 36)
v3.0 approved (recruitment and comp)

Significant Changes to Protocol:
- 2/10 first trial
- v1.1a1 - 2/13 - final update of I1
  - expanded demographics during intake
  - read TLX instructions during intake
  - complete BCS during intake
  - TLX and SUS taken after each phase
- v2.1 - 2/23 - incorporate I2
  - added 2nd investigation
- v3.0 - 4/10 - final version
  - expand recruitment add compensation
  - switch to self-service onboarding (officially)
  - added industry recruitment (paused)
...
::: content-hidden
TODO: cite Monir's dissertation as I2
:::
...
Workstations seven through nine are configured for the first session as seen in @fig-wssetup1. The numbered positions indicate the flow of work in process (WIP) through the system. Assemblies begin at (1) and move, sequentially, to ST-8 (2). When a car is completed or retired (due to breakage considered irreparable by the participant), it is moved to either the green (3a) or red tray (3b), respectively. WIP at (3a) and (3b) is promptly collected and moved to (4) for inspection, after which they are moved to the results tray (not represented). Video recording and HL2 performance are monitored via the PC and iPad represented. This figure is not to scale but does effectively convey the approach used.
...
Complete reports can be reviewed...
:::
...
::: content-hidden
TODO: update these methods based on actual; establish the use of 0.05 for alpha
:::
