...
Participants were instructed to focus on three priorities during the first phase: (1) learn the steps of the assembly process, (2) complete each assembly correctly, and (3) finish as many assemblies as possible in the time allowed. This approach of prioritizing correct and efficient work is in line with measures observed and the principles of OEE, which is used in the second and third phases of this study.[^24-methods-oee-phases]

[^24-methods-oee-phases]: In the final defended analysis, OEE remained the primary effectiveness measure for the recall phase. Retention was ultimately evaluated using changes in task completion time and uncorrected error count, as explained in the corrected H3 retention subsection and Results chapter.
...
The development of the IRB was a collaborative effort which I spearheaded as the Principle Investigator (PI), authoring the majority of the application, and ensuring alignment with the study's goals and ethical standards. Contributions from other members of the research teams, as specified in [Appendix A: Team Contribution Matrix](#app-contribs), were essential to its thorough design and timely approval.

The final approved version, including all supporting materials, is incorporated as [Appendix E: Institutional Review Board Approval](#app-irb). All processes described herein are all based on the approved protocols.
...
::: content-hidden
For more information, see [Appendix C - Session 1 Conduct](#app-s1conduct).
:::
...
::: content-hidden
Complete details, including all categorical labels, can be found in [Appendix G - Data Dictionary](#app-datadict). The final output of this process was saved as `i1_raw_data.xlsx`.
:::
...
A search was conducted, and a variety of tools were tested. Some notes from that process are included in [Appendix B](#app-videotool). In the end, three complementary tools were selected that best fit the requirements identified:
...
A general description of video processing follows. For a step-by-step treatment, see [Appendix C](#app-videoproc) for the original instructions.
...
Based on the primary and secondary research questions identified herein, several types of analysis will be required. Essential methods are summarized in the following list.[^24-methods-analysis-plan]

[^24-methods-analysis-plan]: This list reflects the original planned analysis at the time of writing. The defended Results chapter reports the final analysis path actually used, including non-parametric tests, mixed models, and retention analyses based on changes in TCT and UCE rather than change in OEE.
...
2.  Hypothesis Testing
    ...
    -   One-way ANOVA: Use to compare means of dependent variables (e.g., average time per car, average error count per car, OEE) across the four instructional methods (PWI, PAR, HMDAR, HMDMR).[^24-methods-hmd-labels]

[^24-methods-hmd-labels]: The labels HMDAR and HMDMR refer to the same treatment conditions reported elsewhere in this dissertation more simply as AR and MR, respectively. The shorter forms are used in the Results and Conclusions chapters for readability.
