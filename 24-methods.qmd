...
This chapter provides a detailed account of the study's methodological approach, including the experimental design, data collection procedures, and analysis strategies. It begins with an overview of the key components of the research methodology, followed by a comprehensive description of the measures and variables used to assess learning, recall, and retention outcomes. The experimental procedures are then outlined, including participant recruitment, random assignment to treatment conditions, and the conduct of the training sessions.

The chapter also discusses the study's compliance with ethical guidelines and the steps taken to ensure participant safety and confidentiality. Data extraction and analysis procedures are described in detail, highlighting the use of both quantitative and qualitative methods to gain a holistic understanding of the impact of different IMTs on operator performance and learning outcomes.

By providing a transparent and rigorous account of the research methods employed, this chapter lays the foundation for the subsequent presentation and interpretation of the study's findings. The insights gained from this research have the potential to advance our theoretical understanding of how AR/MR technologies support learning and skill acquisition, while also informing the practical application of these tools in manufacturing training contexts.
...
In this section, we will provide a detailed description of the study design. We will begin by discussing the task and the authentic manufacturing environment in which the experiments are situated. Next, each of the four treatment levels will be described. Finally, the affordance-based nature of this design will be detailed, wherein the underlying affordances will be identified for each treatment and inherent tradeoffs are described.
...
::: content-hidden
LATER: replace bold terms in LG system description
:::
...
### Affordances and Theoretical Benefits {#sec-afford-fw}
...
Learning, recall, and retention are the outcomes of interest in this study. They were chosen for two reasons: (1) they are commonly used in related studies, and (2) they are supported by research in cognitive and educational psychology. But learning, recall, and retention are all psychological *constructs*; abstract concepts or ideas used to explain a phenomenon or behavior [@braun2001role]. They are fundamental to the research questions, but not directly observable, and therefore must be inferred from behaviors, actions, or outcomes.
...
::: content-hidden
MAYBE: discuss construct validity, claim these are
:::
...
Each of the identified constructs can also be assessed using a variety of calculated measures, including Learning Rate (LR), Transfer Effectiveness Ratio (TER), and Overall Equipment Effectiveness (OEE). LR is a measure of the rate of change of learning and can be calculated in a number of ways. TER quantifies the value of time spent training [@roscoe1971incre; @kaplan2021effec], based on the amount of time required to reach certain training outcomes with ($Y_c$) and without ($Y_x$) augmentation, as seen in @eq-ter.

$$\textstyle{TER = \frac{Y_c - Y_x}{Y_c} \times 100}$$ {#eq-ter}

::: content-hidden
MAYBE: describe calculation of LR
:::

OEE has emerged as a fundamental and widely accepted KPI in manufacturing [@ngcorrales2020overa]. Introduced by Seiichi Nakajima [-@nakajima1988intro] as part of Total Productive Maintenance (TPM), OEE evaluates overall manufacturing performance as the product of system availability, productivity, and quality, as shown in @eq-oee. It is instrumental in pinpointing areas for improvement in equipment utilization and production processes.

$$\textstyle{OEE = Availability \times Productivity \times Quality}$$ {#eq-oee}

These three values are the percentage of measured vs expected speed, yield, and up-time, respectively. As seen in @eq-oee-prod, productivity is simply the number of units produced multiplied by the takt time, divided by the operating time. Completed units includes those with errors, but not those retired by the operator.

$$\textstyle{Productivity = \frac{UnitsProduced \times TaktTime}{OperatingTime}}$$ {#eq-oee-prod}

Quality is the number of units produced less the number with errors, divided by number produced. This intuitive calculation is shown in @eq-oee-qual. Once again, retired units are not included in these counts.

$$\textstyle{Quality = \frac{UnitsProduced - DefectiveUnits}{UnitsProdcued}}$$ {#eq-oee-qual}

OEE's third and final component, availability, is commonly calculated by dividing the amount of time that the system was operational (aka measured up-time) by the scheduled time. This is shown in @eq-oee-avail.

$$\textstyle{Availability = \frac{UpTime}{ScheduledTime}}$$ {#eq-oee-avail}
...
### Controlled Factors {#sec-controls}
...
Participants were instructed to focus on three priorities during the first phase: (1) learn the steps of the assembly process, (2) complete each assembly correctly, and (3) finish as many assemblies as possible in the time allowed. This approach of prioritizing correct and efficient work is in line with measures observed and the principles of OEE, which is used in the second and third phases of this study.
...
::: content-hidden
MAYBE: create an accurate image of HL/Unity/MRTK workflow to replace this one
:::
...
::: content-hidden
MAYBE: get a better image of Unity HL2 simulator interface
:::
...
A consistent approach was taken for the HMDMR treatment design, extending the capabilities of the HMDAR treatment. HMDMR used more sophisticated tracking methods to enable freeform interaction with the workpiece, as described in @sec-afford-fw.

### Implementation

::: content-hidden
MAYBE: make project code and data available?
:::
...
::: content-hidden
MAYBE: replace with image of spatial mapping from study
:::
...
::: content-hidden
MAYBE: replace with images from study: VE creator app and work area target
:::
...
![Fixture Masked to Reduce Reflectance](images/24-FixtureMask.png){#fig-fixture width="5in"}

The inherent complexity of MR also led to less reliable tracking and increased instability in that treatment. When tracking is interrupted the system is unable to place virtual objects in the scene. The user experiences this as a "drop-out," where everything disappears from their field of view. Once tracking is reacquired, the virtual objects return. In rare cases of extreme instability, this cycle could cause the system to crash or require a reset. As described in @sec-dropouts, the frequency, duration, and impact of these events varied, and was accounted for.
...
::: content-hidden
MAYBE: replace ST8 start and end images with photos?
:::
...
### First Session Scheduling {#sec-sched}
...
### System Availability {#sec-dropouts}
...
::: content-hidden
MAYBE: include some / all reports in appendix or online?
:::
...
Overall this process was tedious and time consuming but well planned and carefully executed for the given inputs. Future studies of this sort would greatly benefit from a synchronized multi-camera recording setup capable of directly rendering the desired screen layout, saving hours of editing time and eliminating the drift issues.

## Data Cleaning and Analysis {#sec-analysis}
...
::: content-hidden
MAYBE: discuss "tidy" methods
:::

::: content-hidden
MAYBE: detail / cite / footnote packages used
:::
...
::: content-hidden
MAYBE: add chart illustrating data processing workflow
:::
...
::: content-hidden
MAYBE: add details re: parent / child and error correction
:::
...
Based on the primary and secondary research questions identified herein, several types of analysis will be required. Essential methods are summarized in the following list.
...
2.  Hypothesis Testing
    ...
    -   One-way ANOVA: Use to compare means of dependent variables (e.g., average time per car, average error count per car, OEE) across the four instructional methods (PWI, PAR, HMDAR, HMDMR).
