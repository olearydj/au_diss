...
This process was straightforward except as described below. Complete details, including all categorical labels, can be found in [Appendix G - Data Dictionary](#app-datadict).
...
The output of this script was carefully validated against the XML data, video recordings, and reports described next.
...
1.  An Excel file in `xlsx` format, comprised of all transcribed self-reported data and manually recorded results.
2.  *Performance data files exported in `xml` format, containing the annotations from each video.*
3.  Notes for each participant in `md` format, compiled from observations and feedback.

*The focus of the analysis work will involve data in the `xml` and `xlsx` files, but first the performance data (times and events) and results data (outcomes and errors) must be combined. This involves*
...
-   **Software Used**: Mention any specific software tools used for data cleaning.
-   **Preparation for Analysis**: Discuss how the cleaned data is formatted or organized in preparation for analysis.
:::

discuss cleaning / processing the data for analysis

### Validation

Discuss the creation of participant reports for easy access of summarized findings.
...
summarize plans for analysis to be executed in results chapter
