...
The contents of all digitized results were manually transcribed from the PDFs and collected in a single Excel workbook. Each instrument was given a separate tab, with each row representing a participant record. The results from both Data Collection Sheets were combined into a single tab, where each row recorded the outcome of a single assembly task.
...
Summarize the output of the extraction effort.
...
The same file can be used by other researchers interested in further annotating the video.
